...
def split_eval_set(eval_set: list[dict], holdout: float, seed: int = 42) -> tuple[
    list[dict], list[dict]]:
...
def run_loop(
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        test_result_list = [r for r in all_results["results"] if
                            r["query"] not in train_queries_set]

        train_passed = sum(1 for r in train_result_list if r["pass"])
        train_total = len(train_result_list)
        train_summary = {"passed": train_passed, "failed": train_total - train_passed,
                         "total": train_total}
        ...
        if test_set:
            ...
            test_summary = {"passed": test_passed, "failed": test_total - test_passed,
                            "total": test_total}
    ...
    if args.report != "none":
        if args.report == "auto":
            ...
            live_report_path = Path(
                tempfile.gettempdir()) / f"skill_description_report_{skill_path.name}_{timestamp}.html"
        else:
