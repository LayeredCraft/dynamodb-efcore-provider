...
| Type casting                            | `Cast<T>`                                                                    | Not supported; TPH discriminator filtering via `OfType<TDerived>()` and `is` is supported; `GetType()` checks are limited to exact concrete mapped entity types |
