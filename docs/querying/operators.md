...
| `entity.Collection.Any()`                             | `size(attr) > 0`                               | Native DynamoDB primitive collection only; predicate `Any(...)` remains unsupported except inline finite collection rewrites                                                                                                                                                                                                                                                  |
| `entity.Collection.Count`, `.Count()`, `.Length`      | `size(attr)`                                   | Native DynamoDB primitive collection length/count; predicate `Count(...)` and query-level aggregate `Count` remain unsupported                                                                                                                                                                                                                                                |
