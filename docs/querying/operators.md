...
| `WithConsistentRead()` | Requests strongly consistent reads for this query                 | Sets `ExecuteStatementRequest.ConsistentRead = true`. `WithConsistentRead(false)` explicitly uses eventually consistent reads for one query     |
