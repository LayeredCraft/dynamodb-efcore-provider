...
| `entity.Collection[index]`, `.ElementAt(index)`       | `attr[index]`                                  | Direct primitive LIST element access only; filtering, ordering, or paging inside list elements remains unsupported                                                                                                                                                                    |
| `entity.Collection.First()`, `.FirstOrDefault()`      | `attr[0]`                                      | Direct first-element access on primitive LIST attributes only; general sequence `First*` terminal rules still apply to root queries                                                                                                                                                   |
...
| Offset / paging       | `Skip`, `Take`, root/query-sequence `ElementAt`, `ElementAtOrDefault`                | DynamoDB has no offset semantics; direct primitive LIST attribute `ElementAt(index)` translates to list indexing   |
