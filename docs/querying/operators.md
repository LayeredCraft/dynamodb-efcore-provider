...
| `OfType<TDerived>()`                                  | discriminator predicate                        | TPH inheritance only; filters by concrete discriminator values for the requested derived type. Type tests such as `x is TDerived` and `x.GetType() == typeof(TDerived)` also translate to discriminator predicates.                                                                   |
