...
| `==`, `!=`, scalar `Equals`                           | `=`, `<>`                        | Works on all scalar types; supports single-argument `prop.Equals(value)` and `object.Equals(prop, value)` for compatible scalar operands; comparer and `StringComparison` overloads are unsupported |
