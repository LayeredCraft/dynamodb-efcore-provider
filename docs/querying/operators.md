...
| `==`, `!=`, scalar `Equals`                                             | `=`, `<>`                                      | Works on all scalar types; supports single-argument `prop.Equals(value)`, `string.Equals(value, StringComparison.Ordinal)`, static `string.Equals`, and `object.Equals(prop, value)` for compatible scalar operands; complex property-to-property and parameter equality translate for map attributes; comparers and non-ordinal `StringComparison` overloads are unsupported |
...
| `string.Compare(a, b) > 0` (any comparison against 0)                   | `a > b`                                        | Required for string range comparisons; works with `==`, `!=`, `<`, `<=`, `>`, `>=` against `0` only. Exact comparisons against `1` or `-1` are not translated because .NET only guarantees negative/zero/positive results                                                                                                                                                     |
| `a.CompareTo(b) > 0` (any comparison against 0)                         | `a > b`                                        | Same as above; `0 < a.CompareTo(b)` form also supported (operands mirrored). Compare only against `0`                                                                                                                                                                                                                                                                         |
...
| nullable scalar/complex `HasValue`                                      | `IS NOT NULL AND IS NOT MISSING`               | Checks that the scalar attribute or complex map attribute exists and is not `NULL`                                                                                                                                                                                                                                                                                            |
| nullable complex `.Value.Property`                                      | nested attribute path                          | Accesses scalar members inside nullable complex value objects, e.g. `x.Profile.Value.ZipCode` → `"Profile"."ZipCode"`                                                                                                                                                                                                                                                         |
...
| `string.Contains(s)`                                                    | `contains(attr, ?)`                            | Case-sensitive substring; supports `string` and `StringComparison.Ordinal` overloads; `char`, culture, and non-ordinal comparison overloads are unsupported                                                                                                                                                                                                                   |
...
| `entity.Collection.Any()` / `entity.Bytes.Any()`                        | `size(attr) > 0`                               | Native DynamoDB primitive collection and binary (`byte[]`) attributes only; predicate `Any(...)` remains unsupported except inline finite collection rewrites                                                                                                                                                                                                                 |
| `entity.Collection.Count`, `.Count()`, `.Length`; `entity.Bytes.Length` | `size(attr)`                                   | Native DynamoDB primitive collection length/count and binary byte length; predicate `Count(...)` and query-level aggregate `Count` remain unsupported                                                                                                                                                                                                                         |
| `entity.Collection[constant]`, `.ElementAt(constant)`                   | `attr[index]`                                  | Direct primitive LIST element access with a constant numeric index only; dynamic indexes, filtering, ordering, or paging inside list elements remain unsupported                                                                                                                                                                                                              |
| `entity.Collection.First()`, `.FirstOrDefault()`                        | `attr[0]`                                      | Direct first-element access on primitive LIST attributes only; translates as constant index `0`. General sequence `First*` terminal rules still apply to root queries                                                                                                                                                                                                         |
| `string.StartsWith(s)`                                                  | `begins_with(attr, ?)`                         | Case-sensitive prefix; supports `string` and `StringComparison.Ordinal` overloads; `char`, culture, and non-ordinal comparison overloads are unsupported                                                                                                                                                                                                                      |
...
| `entity.Bytes.SequenceEqual(bytes)`                                     | `attr = ?`                                     | Binary (`byte[]`) equality; binary literals are parameterized because DynamoDB PartiQL has no inline binary literal syntax                                                                                                                                                                                                                                                    |
| `collection.Contains(prop)`                                             | `prop IN [?, ...]`                             | In-memory collection membership; max 50 PK values, 100 non-key values. Nullable item comparisons are supported when element mapping is known                                                                                                                                                                                                                                  |
| `new[] { a, b }.Any(x => x == prop)`                                    | `(a = prop) OR (b = prop)`                     | Inline finite collection predicate rewrite only; no server aggregate or subquery is emitted                                                                                                                                                                                                                                                                                   |
| `new[] { a, b }.All(x => x != prop)`                                    | `(a <> prop) AND (b <> prop)`                  | Inline finite collection predicate rewrite only; useful for negated contains-style predicates                                                                                                                                                                                                                                                                                 |
...
| LINQ Operator | PartiQL Translation  | Notes                                                                                                                                                                 |
| ------------- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Select`      | Explicit column list | `SELECT *` is never emitted; computed expressions and nested complex-property shaping evaluate client-side. Primitive collection properties can be projected directly |
...
| --------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| Element operators     | `Any(predicate)`, `All` except supported inline finite collection predicate rewrites | General server-side element operators are unsupported; primitive collection `Any()` translates to `size(attr) > 0`  |
| Offset / paging       | `Skip`, `Take`, root/query-sequence `ElementAt`, `ElementAtOrDefault`                | DynamoDB has no offset semantics; direct primitive LIST attribute `ElementAt(constant)` translates to list indexing |
...
| Aggregation           | `Count`, `LongCount`, `Sum`, `Average`, `Min`, `Max`                                 | No query aggregate functions in DynamoDB PartiQL; primitive collection count/length translates to `size(attr)`      |
