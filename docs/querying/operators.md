...
| `==`, `!=`, scalar `Equals`                           | `=`, `<>`                        | Works on all scalar types; supports single-argument `prop.Equals(value)` and `object.Equals(prop, value)` for compatible scalar operands; complex property-to-property equality translates for map attributes; comparer and `StringComparison` overloads are unsupported |
