...
> exercised a project shape with **TFM-conditioned `PackageReference`s** (`Condition="'$(TargetFramework)' == 'net11.0'"`) — which is exactly what this repo's `Directory.Packages.props` needs, since EF10
...
1. **Synthetic repro** (isolated, InMemory provider, no DynamoDB): `dotnet publish -f net11.0 -c Release` on a multi-targeted project completed the **entire** NativeAOT publish pipeline —
...
2. **Real repository validation** (this branch, `feat/ef11-nativeaot-compatibility`, changes made
    ...
    `tests/EntityFrameworkCore.DynamoDb.NativeAotTests/EntityFrameworkCore.DynamoDb.NativeAotTests.csproj`
    ...
    `dotnet publish tests/.../EntityFrameworkCore.DynamoDb.NativeAotTests.csproj --configuration Release --framework net11.0 --runtime osx-arm64`:

    - Restore succeeded.
    - `OptimizeDbContext` ran and succeeded — confirmed by inspecting the actual generated output:
        `obj/Release/net11.0/Program.EFInterceptors.AotRuntimeContext.g.cs` contains **real, compiled
        query interceptors** (`Query1_Where1`, `PrecompiledQueryContext<TSource>`, etc.), not a
        skipped/no-op step.
    - `obj/Release/net11.0/AotRuntimeItemEntityType.g.cs` references `DynamoTypeMapping<...>` 13 times —
    ...
    - **Not yet run to completion**: actually executing the published binary against DynamoDB Local
        (the full `run-nativeaot-tests.sh` script) — the publish/precompile success (the actual
...
`MSBuildWorkspace` has no special handling for and simply drops per the `// TODO: pass through properties` gap. The precompilation re-open naturally lands on a real, already-restored TFM
...
- Package-version-per-EF-line (§2.1): **preserved**. `VersionPrefix` is already
    ...
    invocation): it fails outright with `MSB1001: Unknown switch. Switch: --framework`. `dotnet build`/`dotnet publish`/`dotnet test` accept `--framework`; `dotnet pack` does not. The correct,
    ...
    **only** the net10.0-conditioned `PackageReference` set as its nuspec dependency group (repeated
    ...
    CI would key off `-p:TargetFrameworks=net10.0`/`net11.0` instead of `--configuration "Release EF10"/"Release EF11"` — a mechanical substitution, not a loss of capability, but **not** the
...
- CI/Taskfile (§2.4): needs updating (see §8, migration complexity) but nothing about the
    *capability* is lost — matrixing over `--framework net10.0`/`net11.0` with `-c Debug`/`Release` is at least as expressive as matrixing over four configuration strings, and is
    the more standard shape for tooling (IDEs, `dotnet` CLI flags, MSBuild target framework
    inference) to understand natively.

**New consideration this alternative introduces**: ordinary multi-targeted `dotnet build`/`dotnet test` (no `--framework` filter) builds **both** TFMs by default — CI/Taskfile commands that
...
- `Directory.Packages.props`: two remaining `$(Configuration)`-conditioned groups
    ...
    path in one place.
- Every `.csproj` currently declaring `<Configurations>Debug EF10;Release EF10;Debug EF11;Release EF11</Configurations>` plus the four TFM-selecting `PropertyGroup`s needs to
    ...
    `SpecificationTests`, the NativeAOT test project, `Example.Simple`) — roughly 8 project files.
...
- CPM, the `.slnx`, `Taskfile.yml`, and `scripts/run-nativeaot-tests.sh` all convert mechanically
    and cleanly.
...
isolated per-TFM assets and dependencies. `publish-preview.yml`'s current `dotnet pack ... --no-build -o artifacts /p:Version=...` step (no TFM filter) would need
...
projects, the NativeAOT test project, `Example.Simple`) to `<TargetFrameworks>net10.0;net11.0</TargetFrameworks>`
was re-checked against the full project inventory (`grep` over every `.csproj`) — confirmed all
share the identical `<Configurations>Debug EF10;Release EF10;Debug EF11;Release EF11</Configurations>` + 4-way `PropertyGroup` pattern, so the same mechanical edit applies
...
`scripts/run-nativeaot-tests.sh` takes `CONFIG` as its first positional argument and passes it
...
`Example.Simple`, all 5 test projects — the NativeAOT test project is not currently in the `.slnx`). Under B,
...
`scripts/run-nativeaot-tests.sh`) rather than the two-project partial conversion in §4.B — found
...
standard `Debug`/`Release`, and updated `Taskfile.yml`/`scripts/run-nativeaot-tests.sh` to use
`--framework`. Full solution restore/build/test passed cleanly on both TFMs (1744 tests, 0
failures). `scripts/run-nativeaot-tests.sh` for **EF10/net10.0 passed completely**, real
end-to-end, including DynamoDB Local execution of all 8 runtime test queries. The same script for
...
with the override in place shows the exact same internal `<MSBuild Targets="Build" Properties="Configuration=...;PublishAot=false;...">` re-invocation that #38955 is about (no
...
- EF11 override: `scripts/run-nativeaot-tests.sh` produced a real ~30MB native binary; a real
    ...
    AOT warning baseline matched exactly (`IL2026 IL2055 IL2060 IL2067 IL2072 IL2075 IL2091 IL2104 IL3050 IL3053`, no new IDs); running the binary against a real `amazon/dynamodb-local:3.3.0`
    container executed all 8 existing runtime test scenarios successfully (generated async query,
...
bypassing the NativeAOT test script's strict exit-code gate, with full success). `net10.0`'s ILCompiler did
...
generation, single-TFM pack, real NativeAOT runtime tests validation before publish) is this repo's own
...
`application-build` can keep delegating to `devops-templates` unchanged, with `buildConfiguration: Debug` (no more per-EF matrix, no devops-templates edit needed). The `native-aot` job becomes a
2-leg `fail-fast: false` matrix (`net10.0`, `net11.0`) — EF11 is finally exercisable here, not just
EF10 — each leg: write override → `scripts/run-nativeaot-tests.sh` → clear override
...
**`publish-release.yaml` and `publish-preview.yaml`**: brought fully local (no `uses: LayeredCraft/devops-templates/.github/workflows/publish-*.yml`). Both keep the *composite*
...
derivation, same `resolve` job shape). Preview: matrix over `{targetFramework, dotnetVersion, drafterConfig, artifact_name}` replaces the old `{configuration, ...}` matrix — release-drafter
dry-run version resolution is unchanged. Both flows insert real NativeAOT runtime tests validation
(`scripts/run-nativeaot-tests.sh`) between build/test and pack, so a NativeAOT regression blocks
