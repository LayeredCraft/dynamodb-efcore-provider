...
- **`tests/EntityFrameworkCore.DynamoDb.NativeAotTests/`** — the only project that references
    ...
    `scripts/run-nativeaot-tests.sh` (publish → AOT-warning baseline gate → Docker-run against
...
- **CI** (`.github/workflows/pr-build.yaml`): `application-build` (Debug EF10 + Debug EF11
    matrix), `native-aot` (**EF10-only**, `Release EF10`, runs the NativeAOT test script), `aot-generation`
...
- **Taskfile**: `test:aot-generation` (CONFIG param), `test:aot-publish` (CONFIG default
    `Release EF10`, shells to the NativeAOT test script), `build:ef11:ci-sdk` (installs a CI-matching .NET 11
    SDK to `.dotnet/ef11`). No EF11-specific `test:aot-publish` task exists; it would have to be
    invoked manually with `CONFIG="Release EF11"`, and is untested/unsupported per the NativeAOT test project's
    `AGENTS.md`.
...
Ran the real NativeAOT test project (`dotnet publish tests/EntityFrameworkCore.DynamoDb.NativeAotTests/... --configuration "Release EF11" --runtime osx-arm64`) at the repo's current pins: **fails**, reproducing the AGENTS.md-documented symptom
...
- **Provider fails to build**, with exactly the two breaks the existing AGENTS.md predicted:
    - `src/EntityFrameworkCore.DynamoDb/Query/Internal/DynamoQueryableMethodTranslatingExpressionVisitor.cs:13`
        — `CS0534: does not implement inherited abstract member 'QueryableMethodTranslatingExpressionVisitor.TranslateFullJoin(ShapedQueryExpression, ShapedQueryExpression, LambdaExpression, LambdaExpression, LambdaExpression)'`.
    ...
    - `src/EntityFrameworkCore.DynamoDb/Design/Internal/DynamoCSharpRuntimeAnnotationCodeGenerator.cs:44`
        — `CS0115: 'Create(CoreTypeMapping, CSharpRuntimeAnnotationCodeGeneratorParameters, ValueComparer?, ValueComparer?, ValueComparer?)': no suitable method found to override`.
...
upstream bug. Related (but not matching) NativeAOT/precompile issues exist (`dotnet-ef optimize --nativeaot` CS9137 workaround via `InterceptorsPreviewNamespaces` in #35945;
...
3. **NativeAOT validation**: extend the existing `native-aot` CI job/`run-nativeaot-tests.sh`
...
4. **`test:aot-generation`/`AotTests.csproj` gap**: this project should reference
    `Microsoft.EntityFrameworkCore.Tasks` (matching the NativeAOT test project) so it actually exercises the
...
- Extend `native-aot` to a matrix job covering `Release EF10` and `Release EF11` (or add a
    parallel EF11 leg) using the same `run-nativeaot-tests.sh` script, parameterized by `CONFIG`.
...
- `tests/EntityFrameworkCore.DynamoDb.NativeAotTests/`: the whole "EF10 ONLY" framing
...
- **CI shape**: extend the existing `native-aot` job to a `fail-fast: false` matrix over
    `Release EF10` / `Release EF11`, reusing `run-nativeaot-tests.sh` as-is (parameterized by
...
'DynamoDbQueryableExtensions'`. The failure is identical for every method in the block, not `Limit\`-specific.
...
- `WithNextToken(...)` failed precompilation. Its translation
    ...
    separate compilation referencing the provider only as a metadata reference (`does not contain a definition for 'ValidateWithNextToken'`).
...
`.Limit(pageSize)`) fails with `System.Diagnostics.UnreachableException: IdentifierName of type ParameterSymbol: pageSize`; referencing a private `const string` field similarly fails with
`InvalidOperationException: Encountered unknown identifier name '...', which doesn't correspond to a lambda parameter or captured variable`. The fix is mechanical and must be applied by every
...
(`DynamoQueryableMethodTranslatingExpressionVisitor.cs`: `'ToPageAsync' cannot be combined with 'Limit'`), since `ToPageAsync(limit, nextToken, ct)` already takes both directly.

Empirically confirmed in the NativeAOT test project: a query with **no** generated interceptor at all
...
`System.InvalidOperationException: Query wasn't precompiled and dynamic code isn't supported with NativeAOT`, aborting the process. This means `ToPageAsync(...)` cannot be used **anywhere** in a
...
NativeAOT test project instead bootstraps a real continuation token via a raw AWS SDK `ExecuteStatementAsync`
