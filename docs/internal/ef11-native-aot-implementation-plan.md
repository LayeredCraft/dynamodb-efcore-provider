...
`AotTests` exercises the provider's generator API in-process; `NativeAotTests` exercises the real
...
- All 8 `.csproj` files (`src/EntityFrameworkCore.DynamoDb`, `tests/*` ×5, `examples/Example.Simple`,
    `tests/EntityFrameworkCore.DynamoDb.NativeAotTests`): replace `<Configurations>Debug EF10;Release EF10;Debug EF11;Release EF11</Configurations>` plus per-configuration `<TargetFramework>`
...
- `Taskfile.yml`: `CONFIG=` vars become `FRAMEWORK=` vars (`net10.0`/`net11.0`, default
    `net10.0`); `task test:aot-publish` generates the override, runs the NativeAOT test script, clears the
    override (`trap ... EXIT` inside the underlying script so a failed run can't leave it behind).
- `scripts/run-nativeaot-tests.sh`: accept a `FRAMEWORK` argument, pass `--framework` to the
...
- **`pr-build.yaml`**: `application-build` needs no override — ordinary multi-targeted build/test
    ...
    override, runs `scripts/run-nativeaot-tests.sh`, clears the override in an `if: always()` step.
...
- **`publish-release.yaml`** and **`publish-preview.yaml`**: become fully local — no more `uses: LayeredCraft/devops-templates/.github/workflows/publish-*.yml`. This provider's EF-major
    ...
    - Preview: matrix over `{targetFramework, dotnetVersion, drafterConfig, artifact_name}` replaces
        ...
        unchanged.
    - Both: `write-target-framework-override.sh <tfm>` → restore/build/test → **real NativeAOT runtime tests
        validation** (`scripts/run-nativeaot-tests.sh`) → pack → upload artifact → clear override
...
- `tests/EntityFrameworkCore.DynamoDb.NativeAotTests/`: the old EF10-only restriction is replaced
    by CI validation of both EF10 and EF11 via the `net10.0`/`net11.0` `native-aot`
    matrix; EF11 requires the exact `11.0.0-rc.1.26425.128` pin.
- Root `AGENTS.md`: remove the `## NativeAOT Runtime Tests — EF10 ONLY` framing and the "Do NOT add an
    EF11 NativeAOT test leg to CI" instruction; document `TargetFrameworkOverride.props` and
...
- `docs/querying/precompiled-queries.md`: remove the "EF Core 11 currently has a known blocker"
    ...
    "Verification" section to describe the passing EF11 NativeAOT runtime tests path. Leave the `.Limit(n)`
...
NativeAOT runtime tests, both TFMs, via the physical override:

```bash
scripts/write-target-framework-override.sh net10.0
scripts/run-nativeaot-tests.sh Release "" net10.0
scripts/write-target-framework-override.sh --clear

scripts/write-target-framework-override.sh net11.0
scripts/run-nativeaot-tests.sh Release "" net11.0
scripts/write-target-framework-override.sh --clear
```
...
- Standard `<TargetFrameworks>net10.0;net11.0</TargetFrameworks>` is checked in; no `Debug EF10`/`Release EF10`/`Debug EF11`/`Release EF11` MSBuild configuration names exist anywhere in
    the repo.
...
- NativeAOT runtime tests pass end-to-end for both `net10.0` and `net11.0` using
    `TargetFrameworkOverride.props`: publish succeeds, AOT warning IDs stay within the reviewed
    baseline, and the NativeAOT test project runs its parameterized, materializing, and `SaveChanges` operations
...
- Real NativeAOT runtime tests validation (not a compile-only check) gates NuGet publication in both
    `publish-preview.yaml` and `publish-release.yaml` — a failing NativeAOT test leg blocks packing and
    publishing.
...
- Updated docs (`AGENTS.md`, the NativeAOT test project, `docs/querying/precompiled-queries.md`) no
