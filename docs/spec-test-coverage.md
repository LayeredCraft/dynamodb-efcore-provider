...
| `ConcurrencyDetectorDisabledTestBase`                |       8 |   ✓    |    ✗    | `ConcurrencyDetector` opt-out; supported `Find`/`SaveChanges`/`Single` rows delegate to upstream, while ordered scans, unconstrained scans, aggregates, and sync-only rows are skipped for DynamoDB limitations                                                                 |
| `ConcurrencyDetectorEnabledTestBase`                 |       8 |   ✓    |    ✗    | `ConcurrencyDetector` opt-in; supported `Find`/`SaveChanges`/`Single` rows delegate to upstream, while ordered scans, unconstrained scans, aggregates, and sync-only rows are skipped for DynamoDB limitations                                                                  |
...
| `CompositeKeyEndToEndTestBase`                       |       3 |   ✗    |    ✗    | PK+SK round-trip covered by DynamoDB-specific integration tests; inherited aggregate/sync uniqueness and three-part composite-key cases are skipped                                                                                                                             |
| `ConvertToProviderTypesTestBase`                     |       2 |   ✗    |    ✗    | Additional enum/provider-type conversion query methods beyond `BuiltInDataTypesTestBase`; inherited FK/navigation-shaped variants are skipped                                                                                                                                   |
| `CustomConvertersTestBase`                           |      29 |   ✓    |    ✗    | Value converter round-trips; scalar DynamoDB converter coverage is split from inherited FK/navigation, ordered-result, owned-type, aggregate, and unsupported query-shape cases, which are explicitly skipped                                                                   |
...
| `EntityFrameworkServiceCollectionExtensionsTestBase` |       3 |   ✗    |    ✗    | EF11-only. Provider service registration is idempotent and expected service lifetimes are covered without a DynamoDB table fixture                                                                                                                                              |
| `TypeTestBase<T, TFixture>`                          |       4 |   ✗    |    ✗    | Scalar save/readback coverage through dedicated DynamoDB classes for bool, numeric types, string, GUID, temporal types, and byte arrays; inherited equality query shapes are skipped when unsupported                                                                           |
...
`TypeTestBase<T, TFixture>` save/readback coverage runs through `SaveChanges` overrides. EF10 and EF11 inherited equality query shapes are explicitly skipped because they rely on query forms the DynamoDB provider does not support. Representative scalar equality behavior is covered by `QueryTypeMappingTable` and simpler query integration tests, but this PR did not add a full replacement equality matrix for every scalar type. Primitive collection aggregate cases are skipped because DynamoDB PartiQL has no `COUNT` aggregate support.
...
| `NorthwindAsTrackingQueryTestBase`     |       5 |   ✗    |    ✓    | `AsTracking()` sync-only inherited rows are skipped because DynamoDB queries are async-only; join/navigation cases remain skipped                                  |
| `NorthwindQueryTaggingQueryTestBase`   |       9 |   ✗    |    ✓    | `TagWith()` inherited rows are sync-only or navigation/join-shaped and are skipped; tags have no DynamoDB PartiQL effect                                           |
| `NorthwindChangeTrackingQueryTestBase` |      17 |   ✗    |    ✓    | Sync-only change-tracking query rows and join-shaped modifier-precedence cases are skipped; materialization/tracking behavior is covered by supported query suites |
...
| `InheritanceComplexTypesQueryTestBase` |       9 |   ✗    |    ✗    | EF11-only. Complex-type filters and projections over inheritance execute; complex collection aggregate subquery is skipped because DynamoDB PartiQL has no `COUNT` aggregate support                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `ComplexPropertiesProjectionTestBase`         |      20 |   ✗    |    ✗    | Complex property projections execute where upstream query shape is directly supportable. Ordered association collection/value-type variants are skipped instead of rewritten; explicit skips cover navigation traversal, SelectMany/list flattening, subquery pushdown, and duplicate-root projection materialization gaps. |
...
DynamoDB has no native date/time types; values are stored as ISO 8601 strings. `DateTimeTranslationsTestBase` now covers constant and parameter equality shapes that map to string equality. No additional temporal translation test classes are currently queued here; date-part, current-time, arithmetic, and conversion-heavy temporal bases remain skipped below.
...
30. `InheritanceComplexTypesQueryDynamoTest` — 9 methods (EF11 only)
31. `EntityFrameworkServiceCollectionExtensionsDynamoTest` — 3 methods (EF11 only)
32. `Types/Dynamo*TypeTest` — 4 methods per EF11 scalar class, explicit skipped EF10 equality decisions
...
2. Remaining unsupported math/temporal translation families — after provider feature work
...
Implemented totals reflect the EF11/max surface. EF10 omits the two EF11-only bases above, for 42 implemented base rows / 1,363 methods.
