...
| `FunkyDataQueryTestBase`   |      19 |   ✗    |    ✗    | Edge-case string data with wildcard-like characters; `Contains` and non-null `StartsWith` cases execute, while null-argument `StartsWith` branches, column cross-products, `EndsWith`, and unsupported character operators are excluded or explicitly skipped                                                                                                             |
...
| Other Query           |  3 classes / 145 methods |              — |  8 classes / 310 methods | 18 classes / 1,691 methods |
...
25. `FunkyDataQueryDynamoTest` — 19 methods
