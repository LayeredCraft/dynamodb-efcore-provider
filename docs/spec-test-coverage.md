...
| `PrimitiveCollectionsQueryTestBase` |     172 |   ✓    |    ✗    | Primitive collection fixture is mapped for DynamoDB LIST attributes. Current coverage runs 37 supported methods across inline/parameter `Contains`, MemoryExtensions/ListInit `Contains`, scalar and value-converted collection parameters, column `Contains`/`Any`/`Count`/`Length`, and direct list indexing/first-element access; detailed audit in `docs/spec-test-primitive-collections-audit.md` tracks remaining provider gaps and true long-term blocks. |
