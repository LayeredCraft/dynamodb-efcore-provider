...
`DynamoIntTypeTest` starts `TypeTestBase<T>` coverage for scalar type behavior with `int`: equality query coverage runs on EF10/EF11, EF11 save/readback coverage runs, and the primitive collection aggregate case is skipped because DynamoDB PartiQL has no `COUNT` aggregate support.
...
1. `TypeTestBase` — generic scalar type fixture matrix; `DynamoIntTypeTest` covers the first scalar fixture, remaining scalar fixtures need separate support decisions
