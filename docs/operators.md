...
| `string.StartsWith(string)` | `begins_with(attr, ?)` | N/A | Literal prefix match; `StringComparison` and `char` overloads are not translated |
...
**Translation**
- `entity.Name.StartsWith("prefix")` translates to `begins_with("Name", ?)`.
...
**Limitations / DynamoDB quirks**
- Only `string.StartsWith(string)` is supported; `StringComparison` and `char` overloads are not translated and will throw.
