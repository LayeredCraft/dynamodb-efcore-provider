...
/// <summary>
///     Integration tests for provider-managed <c>$version</c> optimistic concurrency. Covers
///     INSERT duplicate-key errors, stale-version conflicts on UPDATE and DELETE, and correct version
///     propagation across consecutive saves.
/// </summary>
    ...
    : SaveChangesTableTestBase(fixture)
{
    ...
    /// <summary>After a successful UPDATE, the <c>$version</c> attribute in DynamoDB advances by one.</summary>
    [Fact]
    public async Task UpdateAsync_VersionIncrements_AfterSuccessfulSave()
    {
        var customer = new CustomerItem
        {
            Pk = "TENANT#CONC",
        ...
        var itemAfterInsert = await GetItemAsync(customer.Pk, customer.Sk, CancellationToken);
        itemAfterInsert!["$version"].N.Should().Be("1");
    ...
    /// <summary>
    ///     Two sequential SaveChanges calls on the same entity both succeed and the version advances
    ///     correctly from 1 → 2 → 3 across the saves.
    /// </summary>
    [Fact]
    public async Task UpdateAsync_ConsecutiveSaves_TrackVersionCorrectly()
    {
        var customer = new CustomerItem
        {
            Pk = "TENANT#CONC",
    ...
    /// <summary>An UPDATE with the correct version succeeds without exception.</summary>
    [Fact]
