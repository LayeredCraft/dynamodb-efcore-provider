...
{
    /// <summary>Compares two dictionaries by key and value regardless of insertion order.</summary>
    private static bool Equals(TDictionary? left, TDictionary? right, ValueComparer elementComparer)
    {
        ...
        foreach (var pair in leftDictionary)
            if (!rightDictionary.TryGetValue(pair.Key, out var rightValue)
                || !elementComparer.Equals(pair.Value, rightValue))
                return false;
    ...
    private static int GetHashCode(TDictionary? value, ValueComparer elementComparer)
    {
        ...
        foreach (var pair in dictionary)
            hash ^= HashCode.Combine(pair.Key, elementComparer.GetHashCode(pair.Value!));
        ...
        bool readOnly)
    {
        ...
        foreach (var pair in source)
            snapshot[pair.Key] = (TValue)elementComparer.Snapshot(pair.Value!)!;
