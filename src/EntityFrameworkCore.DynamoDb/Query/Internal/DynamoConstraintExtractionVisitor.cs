...
internal sealed class DynamoConstraintExtractionVisitor
{
    ...
    private void ClassifyOr(SqlBinaryExpression orExpr)
    {
        ...
        // non-PK predicate).
        var pkNames = branches.Select(GetSinglePkEqualityPropertyName).ToList();
        ...
        var anyHasPk = pkNames.Any(name => name is not null) || branches.Any(BranchTouchesPk);
    ...
    private bool BranchTouchesPk(SqlExpression branch)
    {
        ...
        {
            SqlBinaryExpression { OperatorType: ExpressionType.AndAlso } and => BranchTouchesPk(
                    and.Left)
                || BranchTouchesPk(and.Right),
            SqlBinaryExpression { OperatorType: ExpressionType.OrElse } or => BranchTouchesPk(
                    or.Left)
                || BranchTouchesPk(or.Right),
            ...
            SqlBinaryExpression bin => (bin.Left is SqlPropertyExpression lp
